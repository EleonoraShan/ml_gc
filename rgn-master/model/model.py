...
class RGNModel(object):
    ...
    def _start(self, evaluation_models, session=None, restore_if_checkpointed=False):
    ...
    if config['higher_order_layers']:
        ...
        for layer_idx in range(num_layers):
            with tf.variable_scope('layer' + str(layer_idx)):
                ...
                if (residual_n is not None) and (residual_n >= 1) and ((layer_idx - residual_shift) % residual_n == 0) and (layer_idx >= residual_n + residual_shift):
